/**
 * Inline SVG icons for the plain-DOM editor chrome (bubble menu, slash menu,
 * block handle). The fixed React toolbar uses `lucide-react`; these mirror the
 * same lucide glyphs as strings so ProseMirror plugin views can stay
 * framework-free (consistent with the NodeView layer).
 */

const svg = (inner: string) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${inner}</svg>`;
...
export const icons = {
  ...
  sigma: svg(
    '<path d="M18 7V5a1 1 0 0 0-1-1H6.5a.5.5 0 0 0-.4.8l4.5 6a.5.5 0 0 1 0 .6l-4.5 6a.5.5 0 0 0 .4.8H17a1 1 0 0 0 1-1v-2"/>',
  ),
  workflow: svg(
